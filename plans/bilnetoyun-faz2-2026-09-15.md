...
## B5 — `games/_shared/edu-kit.js` + 21 eğitsel oyunun taşınması  `[x] 16 Eyl 2026 — dal faz2/b5-edu-kit (PR sahip açar); MUTASYON: depoda 20 eğitsel oyun (21 tasarım sayısı; Bilgi Zıplaması 8977765 ile kaldırılmıştı) — 20/20 taşındı, oyun başına 1 commit; tone imzası oyunların (f, durSaniye, type=triangle, vol, when) — ms/sine taslağı uygulanmadı (kanit/B5-farklar.md #1); ölçüm: `<script>` 20 oyunda 0 (kalan: bil-ve-fethet 2, zindan-okcusu 2, kelime-madeni-3d 2, hava-hokeyi 1 = B9 envanteri), `function tone` yalnız _shared/edu-kit.js, üretilmiş index.html yok; lint 0 hata, smoke 61/61 (8773), test:edu-kit 63/63 (kök + .build-check: hub iframe + bağımsız + CSP provası script-src self → satır içi ihlal 0, prova kontrolü 1), test:build 8/8 (yeni: göreli game.js/../_shared hash testi), build --check OK (game.js?h= 20/20), catalog:check OK, seo test OK; 5 oyun elle tur kanit/B5-tur.md; ci.yml test:edu-kit adımı; ?v= artırılmadı`
...
4. Wrapper `?v=` artırılır (A10b hash build geldiyse gereksiz). `[x]` gereksiz: A10b build `game.js?h=`, `../_shared/edu-kit.js?h=`, `../_shared/edu-kit.css?h=` üretiyor (tests/build.spec.js 8. test).
