...
## B0 — Yön: tasarım sözleşmesi (`docs/tasarim-sozlesmesi.md`)  `[x] 16 Eyl 2026 — Claude Design çıktısı (design/2026-09-16-claude-design/, 14 ekran + token tablosu + bileşen envanteri + 57 oyun görseli v2 + 6 kategori simgesi v2 + maskot "Bulut Bili") → sözleşme yazıldı; kullanıcı yönü Design'da onayladı. MUTASYON: token adları tuvaldeki Türkçe adlar (--mavi-500, --kat-harf, --yazi-kart…); aşağıdaki --paper/--ink/--subject-* taslak adları KULLANILMAZ. Kadran/skill ön-uçuşu Design'da yapıldı; §11.B emekli desen listesi sözleşme §0 "Mod" satırında.`
...
## B1 — `css/tokens.css` + hub kabuğunun token'a geçişi  `[x] 16 Eyl 2026 — dal faz2/b1-tokens (PR sahip açar); ölçüm: 9 hub dosyasında ham hex 0 (tokens.css'te tam 24), transition: all 0, #667eea/#764ba2/--ease-bounce/--ease-spring 0, tanımsız token yalnız --cat-color; lint 0 hata, smoke 60/60, catalog:check OK, build --check OK, Lighthouse a11y 100 (giriş/hub/landing); kanıt kanit/B1-audit.md + B1-*.png`

**Durum 2026-09-16 (kapanış) — MUTASYON:** görev 3'teki "52 `--<oyun>-color` → 7 `--subject-*`" uygulanmadı; sözleşme adları geçerli: 52 ad `tokens.css` köprü katmanında **6 `--kat-*`** takma adıdır (B2a sonrası `js/app.js` zaten `var(--kat-<section>)` okur; köprü yalnız `css/tuval.css`, `css/emoji-yapici.css` ve modül sözleşmesi için, B4'te kaldırılır). `--ease-bounce/--ease-spring` kullanımları `--egri-out`; yalnız yıldız yapışması `--egri-yay`. Sözleşmeden sapmalar (gerekçeli): kategori dolgusu üstünde metin daima mürekkep (beyaz 2.4:1); doğru/yanlış hücreler zemin tonu + mürekkep harf + renkli kenar (doğru yeşilinde beyaz 4.2:1 küçük metinde AA geçmiyor); tuvaldeki "2 oyuncu" rozeti mor dolgu yerine mavi-050 zemin + mürekkep (13 px beyaz 4.4:1). Landing/statik sayfalar (`css/landing.css`) hub'la aynı gökyüzü + beyaz çıkartma kartı; kırıntı/altbilgi gök üstünde mürekkep. `css/admin.css` koyu tema kaldırıldı (§3.14 açık tema, kontursuz). Bağlantı: `index.html`, `admin.html`, `404.html`, `seo/build_seo.py HEAD_COMMON` → `tokens.css` ilk stylesheet. `CLAUDE.md`/`README.md` token satırları güncellendi.
...
## B3 — Modal / dialog sistemi  `[x] 16 Eyl 2026 — dal faz2/b3-dialog (PR sahip açar); ölçüm: js/dialog.js tek API (open/close/isOpen/fromKeyboard), 4 katman taşındı, katmanlarda özel Escape/Tab kodu 0 (`grep "=== 'Escape'" js/*.js` → yalnız dialog.js); tests/dialog.spec.js 14/14 (yedek yol dahil, aynı Chromium'da showModal/inert silinerek); Lighthouse a11y 100 × 4 (giriş, hub, seviye tamamlama açık, meta panel açık — kanit/B3-lighthouse.json); lint 0 hata; smoke 60/60; catalog:check OK; build --check OK; kanıt kanit/B3-a11y.md + B3-*.png`

**Durum 2026-09-16 (kapanış) — MUTASYON:** (1) `kanit/B3-nvda.md` yerine `kanit/B3-a11y.md`: NVDA elle tur **yapılmadı — sahip tarafı**; ağaç Playwright `ariaSnapshot` + `getByRole('dialog', {name})` ile doğrulandı. (2) Kaydırma kilidi için `MobileUtils.lockBodyScroll` **kullanılmadı**: o `body.game-active` bayrağıdır (oyun durumu; seviye tamamlama kapanınca unlock süren oyunun mobil kilidini bozardı); html/body zaten `overflow:hidden`, `.hub` inert, perde `overscroll-behavior:contain` — ayrıca kilit gerekmedi. (3) Hesap menüsü de taşındı: `Dialog.open(menu, { modal:false })` (`<dialog>` + `show()`, perde/inert yok, dışarı tık + odak kaçışı kapatır — WCAG 1.4.13), auth.js'te Escape kodu kalmadı. (4) Tab sarması yerel `showModal` yolunda da modülde: tarayıcı odağı belgede kilitler ama sarmaz (son öğeden Tab adres çubuğuna) — APG deseni için uçlarda sarılır. (5) `#particles-canvas` `data-dialog-ustu`: modal açıkken popover ile üst katmana alınır (konfeti perdenin altında kalmasın); yan kazanım: tarayıcı tam ekranında seviye tamamlama artık görünür. (6) Meta panel kilitli rozet etiketi 1.92:1 idi (Lighthouse 94) → yalnız simge sönük, etiket tam kontrast (100). (7) `eslint.config.js`'e `Dialog: 'readonly'` **eklenemedi** (ECC config-protection hook'u düzenlemeyi engelliyor; ajan hook'u kapatamaz) → dört tüketici dosyada `/* global Dialog */`, dialog.js'te `/* exported Dialog */`; sahip tek satırı `hubCoreGlobals`'a ekleyip yönergeleri silebilir. (8) Yıldız SVG'leri `aria-hidden`, `#complete-stars` `role=img` "3 üzerinden N yıldız" (4.1.2) — animasyona dokunulmadı (B4). Bilinen sınır: popover API'siz tarayıcıda (Safari ≤ 16, Firefox ≤ 124) konfeti perde altında sönük; `<dialog>`'suz tarayıcıda yedek yol.
...
## B8a — Satranç taşları yerel + kütüphane vendor / SRI  `[x] 16 Eyl 2026 — görev 1 PR #36; görev 2–3 dal faz2/b8a-vendor-sri (PR sahip açar); ölçüm: satranc + lego-world cdnjs/unpkg isteği 0 (yalnız three.min.js cdnjs, B6'ya kadar), THREE.GLTFLoader tanımlı, console 0; SRI negatif test: bozuk integrity → tarayıcı script'i engelledi, FIREBASE_OK=false, A4 çevrimdışı bandı görünür; sri:check bozukken 1 / düzgünken 0; lint 0 hata, smoke 60/60 (kök + .build-check), catalog:check OK, build --check OK, seo test 42/42; kanıt kanit/B8a-sri.md + B8a-sri-bozuk.png`
...
2. `[x]` 16 Eyl 2026 — chess.js (15 KB) → `js/lib/chess.0.10.3.min.js`, GLTFLoader r128 → `js/lib/GLTFLoader.r128.js` (+ `js/lib/README.md` sha256/lisans tablosu, `js/lib/LICENSES/`); `data/games.json` yolları yerel → `npm run catalog`; CSP `script-src`'den **unpkg düştü**, cdnjs three.min.js için kaldı (B6 sonrası düşer; `tools/build-catalog.js` `CDN_HOSTS` de yalnız cdnjs); `gizlilik` listesinden unpkg çıktı, cdnjs "yalnız LEGO World için three.js". Plan metni: three B6'daki `js/lib/three.r128.min.js`.
3. `[x]` 16 Eyl 2026 — gstatic ACAO `*` doğrulandı → üç script'e `integrity` (sha384) + `crossorigin="anonymous"` (`index.html` + `admin.html`); `tools/sri-check.js` + `npm run sri:check` + ci.yml adımı; negatif test kanit/B8a-sri.md. Özgün metin: Firebase compat (gstatic): önce `curl -sI https://www.gstatic.com/firebasejs/10.14.1/firebase-app-compat.js | grep -i access-control-allow-origin` → `*` ise `integrity="sha384-…"` (`openssl dgst -sha384 -binary | openssl base64 -A`) + `crossorigin="anonymous"` üç dosyaya; `tools/sri-check.js` (yeni) hash'leri yeniden hesaplayıp karşılaştırır, `ci.yml`'e eklenir (hash uyuşmazlığı hub'ı A4 çevrimdışı moduna düşürür — kırılma değil ama görünmez; CI erken uyarır). ACAO yoksa ya da Karar 5 "vendor" derse üç dosya `js/lib/firebase/` altına.
...
2. `vercel.json` Report-Only değeri nihai: `script-src 'self' 'wasm-unsafe-eval' https://www.gstatic.com https://apis.google.com` (unpkg B8a ile düştü; cdnjs B6 three.js tekilleşince düşer — B8a görev 2 notu; Firebase gstatic + SRI kaldı, Karar 5); `tests/csp.spec.js` (yeni): 60 sayfada `console` mesajı "Content Security Policy" içeren 0 — `BASE_URL=<önizleme>` ile koşar (Vercel önizleme başlıkları prod ile aynı).
...
5. **B8a Firebase SDK:** gstatic + SRI mi, `js/lib/firebase/` vendor mı? (Okul ağı filtresi ve tek origin lehine vendor; güncelleme elle.) → **gstatic + SRI (16 Eyl 2026, sahip adına öneri)**: ACAO `*` doğrulandı, hash uyuşmazlığı A4 çevrimdışı moduna düşürür (kırılma yok), `sri:check` CI'da; vendor gerekirse `js/lib/firebase/` yolu açık kalır.
...
`docs/inceleme-2026-09-15/kanit/` — `B0-kontrast.md`, `B1-audit.md`, `B1-*.png`, `B2b-*.png`, `B3-a11y.md` (NVDA yerine; sahip turu bekliyor), `B3-lighthouse.json`, `B3-*.png`, `B4-hareket.md`, `B4-performance.png`, `B5-farklar.md`, `B6-three.md`, `B7-emulator.md`, `B7-temizlik.log`, Lighthouse JSON'ları. Kabul geçidi bu klasörsüz "geçti" demez.
