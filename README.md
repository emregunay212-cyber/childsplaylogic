...
| `js/auth.js`, `js/firebase-config.js`, `js/multiplayer.js`, `js/lobby.js` | Google girişi / misafir modu, `users/{uid}/gameSaves` bulut senkronu (`GAME_SAVE_KEYS`), Firebase **Realtime Database** lobi/oda. `js/firebase-config.js` = config'in TEK yeri + `window.BilnetBridge` (B6): iframe oyunları (kelimelik, son-kart, ates-buz, hava-hokeyi) SDK'yı yüklemez, `parent.BilnetBridge.ready()` ile hub'ın db'sini alır; bağımsız açılışta online yok (sözleşme `CLAUDE.md`) | Firestore yok |
...
| `js/lib/` | Vendor kütüphaneler, **değiştirilmemiş** kopyalar: `three.r128.min.js` (MIT; LEGO World + Kelime Madeni 3D tek kopya, B6), `chess.0.10.3.min.js` (BSD-2), `GLTFLoader.r128.js` (MIT), `stockfish.js` (GPL-3); `README.md` kaynak URL + sha256 + lisans tablosu, `LICENSES/` metinler (B8a). CDN yok: `data/games.json files` dış URL kabul etmez | Lint dışı; build `?h=` ekler; Firebase SDK vendor değil — gstatic + SRI |
...
npm run test:bridge                     # BilnetBridge (B6): kelimelik/son-kart SDK'yı hub'dan alır (iframe'den gstatic isteği 0), bağımsız açılışta online kapalı + hub bağlantısı, three tek kopya, 4 iframe sayfası CSP satır içi envanteri (12 test)
BRIDGE_LIVE=1 PORT=8782 npm run test:bridge-live   # canlı RTDB, iki cihaz (browser.newContext ×2): Son Kart bir el, Kelimelik 1 tur, hava-hokeyi/ates-buz lobi → iframe köprü yazımı (4 test; CI'da koşmaz)
...
- `vercel.json` CSP **Report-Only** — zorlayıcı moda geçiş B9'da; `script-src` dış origin yalnız `https://www.gstatic.com` + `https://apis.google.com` (B6: cdnjs de düştü). Satır içi `<script>` taşıyan iframe sayfası B6 sonrası 4 (bil-ve-fethet, zindan-okcusu, kelime-madeni-3d, hava-hokeyi — B9 envanteri); 20 eğitsel oyun + kelimelik/son-kart/ates-buz `script-src 'self'` altında ihlalsiz (`npm run test:edu-kit` + `npm run test:bridge` CSP provası).
- Üçüncü taraf script (B8a): Firebase compat SDK gstatic'ten `integrity="sha384-…" crossorigin="anonymous"` ile yüklenir (Karar 5: vendor değil, SRI); hash uyuşmazsa tarayıcı script'i engeller, hub A4 çevrimdışı modunda açılır (tek kişilik oyunlar), CI `sri:check` kırmızı olur. chess.js, GLTFLoader ve three r128 `js/lib/` altında yerel (B6); unpkg ve cdnjs CSP'den düştü — **üçüncü taraf script origin'i yalnız gstatic + apis.google.com**. Firebase config (herkese açık istemci anahtarı) yalnız `js/firebase-config.js`'te; iframe oyunları onu kopyalamaz, `window.BilnetBridge` kullanır.
