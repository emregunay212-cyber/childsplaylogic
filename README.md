...
| `js/games/<id>.js` | Oyun modülü. Ya hub içinde doğrudan çalışır (`const id = '<slug>'`, `init/destroy`) ya da `games/<id>/index.html`'i iframe'e gömen sarmalayıcıdır (örn. `js/games/bilgi-ciftligi.js`). Balon Labirenti üç dosyadır: `balon-labirenti-fizik.js` (saf, deterministik fizik) + `balon-labirenti-levels.js` (30 labirent + `cozum`) + modül; `tools/balon-labirenti-coz.js` çözüm tarayıcı | 65 modül |
...
| `seo/build_seo.py` | Üretici: `oyunlar/<slug>/index.html` (58), `oyunlar/index.html` (B2b §3.11: yaş × kategori süzgeci JS'siz, radio + CSS `:has()`; kartlar hub anatomisinde, kazanım hep açık), `sitemap.xml`, `llms.txt` | **Üretilenler elle düzenlenmez** |
