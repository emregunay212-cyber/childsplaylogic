...
const Penalti = (() => {
    ...
    function render() {
        ...
        shotInfo.className = 'pen-shot-info';
        shotInfo.textContent = T('pen.shot', { n: Math.min(shotsTaken + 1, TOTAL_SHOTS), m: TOTAL_SHOTS });
        const scoreInfo = document.createElement('span');
        scoreInfo.className = 'pen-score-info';
        scoreInfo.textContent = T('pen.goals', { n: goals });
        topBar.appendChild(shotInfo);
        topBar.appendChild(scoreInfo);
    ...
    function showLose() {
        ...
        over.className = 'kod-gameover';
        const card = document.createElement('div');
        card.className = 'kod-gameover-card';
        card.style.background = 'linear-gradient(135deg, #fd79a8, #e17055)';
        const icon = document.createElement('div');
        icon.className = 'kod-gameover-icon';
        icon.textContent = '😔';
        const title = document.createElement('h2');
        title.className = 'kod-gameover-title';
        title.style.color = 'white';
        title.textContent = T('pen.lose_title');
        const sub = document.createElement('p');
        sub.className = 'kod-gameover-sub';
        sub.style.color = 'rgba(255,255,255,0.8)';
        sub.textContent = T('pen.lose_sub', { n: goals, m: TOTAL_SHOTS });
        const retry = document.createElement('button');
        retry.className = 'kod-gameover-btn';
        retry.textContent = T('pen.retry');
        retry.onclick = () => {
            SDK.gameplayStart();
