/* OYUN: Bilim Dedektifi — gizli obje + fen quizi (Faz 3 / 4.4)
   Eğitsel seri — bağımsız tek-dosya HTML oyunu (games/bilim-dedektifi/index.html),
   iframe ile gömülür; kayıt window.storage → localStorage + gameSaves senkronu.
   Yıldız vermez (eğitsel seri kilitsiz). */

const BilimDedektifi = (() => {
    const id = 'bilim-dedektifi';
    const levels = [{}];
    ...
    function init(gameArea, level, callbacks) {
        ...
        iframe.src = 'games/bilim-dedektifi/index.html?v=1';
        iframe.className = 'bd-iframe';
