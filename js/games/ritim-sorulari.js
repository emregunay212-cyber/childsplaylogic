/* OYUN: Ritim Soruları — Piano Tiles + pentatonik melodi (Faz 4 / 4.9)
   Eğitsel seri — bağımsız tek-dosya HTML oyunu (games/ritim-sorulari/index.html),
   iframe ile gömülür; kayıt window.storage → localStorage + gameSaves senkronu.
   Yıldız vermez (eğitsel seri kilitsiz). */

const RitimSorulari = (() => {
    const id = 'ritim-sorulari';
    const levels = [{}];
    ...
    function init(gameArea, level, callbacks) {
        ...
        iframe.src = 'games/ritim-sorulari/index.html?v=1';
        iframe.className = 'rs-iframe';
