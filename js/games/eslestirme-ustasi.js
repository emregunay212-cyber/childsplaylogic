/* OYUN: Eşleştirme Ustası — ilişkili çift hafıza kartları (Faz 4 / 4.11)
   Eğitsel seri — bağımsız tek-dosya HTML oyunu (games/eslestirme-ustasi/index.html),
   iframe ile gömülür; kayıt window.storage → localStorage + gameSaves senkronu.
   Yıldız vermez (eğitsel seri kilitsiz). */

const EslestirmeUstasi = (() => {
    const id = 'eslestirme-ustasi';
    const levels = [{}];
    ...
    function init(gameArea, level, callbacks) {
        ...
        iframe.src = 'games/eslestirme-ustasi/index.html?v=1';
        iframe.className = 'eu-iframe';
